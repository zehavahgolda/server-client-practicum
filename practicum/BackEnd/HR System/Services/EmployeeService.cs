    ...
    public class EmployeeService : IEmployeeService
    {
            ...
            string? search = null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(managerName))
                filtered = filtered.Where(e => string.Equals(e.ManagerName, managerName, StringComparison.OrdinalIgnoreCase));

            if (!string.IsNullOrWhiteSpace(professionalCategory))
                filtered = filtered.Where(e => string.Equals(e.ProfessionalCategory, professionalCategory, StringComparison.OrdinalIgnoreCase));
            ...
            if (!string.IsNullOrWhiteSpace(search))
            {
                var s = search.Trim();
                filtered = filtered.Where(e =>
                    (!string.IsNullOrWhiteSpace(e.FullName) && e.FullName.Contains(s, StringComparison.OrdinalIgnoreCase)) ||
        ...
        public async Task<bool> AddAllocationAsync(string employeeId, AllocationCreateDto dto)
        {
            ...
            var remaining = employee.YearlyCapacityMonths - allocations.Sum(a => a.ActualMonths);
            ...
            var res = await _employeesCollection.UpdateOneAsync(
                e => e.Id == employeeId,
        ...
        public async Task<BulkAssignEmployeesResultDto> BulkAssignEmployeesToSystemAsync(BulkAssignEmployeesDto dto)
        {
            var errors = new List<string>();

            if (dto is null)
                return new BulkAssignEmployeesResultDto(false, 0, new List<string> { "Missing request body." });
            ...
            var systemExists = await _systemsCollection.Find(s => s.Id == dto.SystemId).AnyAsync();
            if (!systemExists)
            ...
            var employeeIds = dto.Employees.Select(e => e.EmployeeId).Distinct().ToList();
            ...
            if (!updates.Any())
                return new BulkAssignEmployeesResultDto(false, 0, new List<string> { "No valid employees to assign." });
